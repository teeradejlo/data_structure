...
void tree_recursive_insert(Tree_CS *T, int root, int map[200][200]) {
    if(map[root][0] == -1) {
        return;
    }
    int i = 0;
    while (map[root][i] != -1) {
        tree_insert(T, root, map[root][i], map[root][i]);
        tree_recursive_insert(T, map[root][i], map);
        i++;
    }
}
...
int main(int argc, char *argv[]){
	...
	tree_init(&T, root, root);
	tree_recursive_insert(T, root, map);
