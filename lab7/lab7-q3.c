...
int maxheap_insert(MaxHeap *H, double key, char* value) {
	/* Your code here */
    int index, parent, i;
...
void maxheap_print(MaxHeap *H, FILE* fout) {
    int i;
    if (H->count == 0) return;
    for (i = 1; i < H->count; i++) {
...
int main(int argc, char *argv[]){
	...
	/* Your code here */
    maxheap_heaplify(&H, S, n);

	/* Output format, you may need to edit this */
    maxheap_print(H, fout);
